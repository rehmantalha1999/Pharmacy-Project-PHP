...
?>
            <?php foreach ($medicines as $medicine): ?>
                ...
                <?php
$expDate = DateTime::createFromFormat("Y-m-d", $medicine['expiry']);
$formatted_exp = date("d-m-Y", strtotime($medicine['expiry']));
$year = explode("-", $formatted_exp)[2];
$month = explode("-", $formatted_exp)[1];
$day = explode("-", $formatted_exp)[0];
$yearDiff = (int) date('Y') - (int)$year;
$monthDiff = (int) date('m') - (int)$month;
$dayDiff = (int) date('d') - (int)$day;
$expired = '';
if($yearDiff>0){
    $expired = 'yes';
}
...
?>
                <td <?= $expired=== 'yes' ? 'class=  error' : "class = ''"?>><?=$formatted_exp?></td>
